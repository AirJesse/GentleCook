...
<body>
				...
				<div class="cook-list">
					...
					<div class="cook-item hidden">
						...
						<div class="cook-operation">
							<a name="comment" href="" class="btn btn-primary btn-lg">查看评论</a>
	...
	<script>
		...
		$.post('cook/getcooks.pub',function(data){
			for(var i in data){
				var cook = data[i];
				$model.find('.cook-avatar').attr('src',data.avatar);
				$model.find("span[name='id']").html(data.id);
				$model.find("span[name='name']").html(data.name);
				$model.find("span[name='gender']").html(data.gender);
				$model.find("span[name='motto']").html(data.motto);
				$model.find("span[name='skill']").html(data.skill);
				$model.find("span[name='acclaim']").html(data.acclaim);
				$model.find("span[name='price']").html(data.price);
				$model.find("span[name='times']").html(data.times);
				$model.find("a[name='comment']").attr("cook-id",data.id)
				$model.find("a[name='order']").attr('href',"order?id="+data.id);
